...
#%%%  PAXOS HERE  %%%
		tmpData =[]
		dict_data = {'NID'3,'nodekey':NodeKey,'OID':2,"walletID":session['walletID']}
...
#%%%  PAXOS HERE  %%%
			dict_data = {'NID':3,'nodekey':NodeKey,'OID':1}
...
# %%%   PAXOS HERE  %%%
			...
			dict_data={'NID':3,'nodekey':NodeKey,'OID':3, 'walletID':session['walletID'],'name':session['name'],
																	'amount':session['amount'],'password':request.form['password']}

			tmpData={}																
			tmpData[2] = paxos(2,dict_data)
			tmpData[1] = paxos(1,dict_data)

			if tmpData[2] == 'OK' or tmpData[1] == 'OK':
				#consensus
				...
				dict_data = {'NID':3,'nodekey':NodeKey,'OID':12,'rollback':tmpCommand}
				paxos(2,dict_data)
				paxos(1,dict_data)
				session.clear()
...
#%%%  PAXOS HERE  %%%
			...
			if data in [None,'Fail']:
				dict_data = {'NID':3, "nodekey":NodeKey, "OID":4,'walletID':request.form['walletID'],'password':request.form['password']}

				data = paxos(2,dict_data)
				if data not in [None,'Fail']:
					data = json.loads(data)
				else:
					data = paxos(1,dict_data)
					if data not in [None,'Fail']:
						data =json.loads(data)
#%%%  PAXOS HERE  %%%
...
#%%%  PAXOS HERE  %%%
				dict_data = {'NID':3, "nodekey":NodeKey, "OID":5,'walletID':session['walletID']}
				tmpData = {}
				tmpData[2]= paxos(2,dict_data)
				tmpData[1]= paxos(1,dict_data)
...
#%%%   PAXOS HERE   %%%
	...
	rows = {}
	rows[3] = cursor.fetchall()
	if rows[3] is ():
		lastTransID=0
	else:
		lastTransID=int(rows[3][0][0])

	dict_data = {'NID':3, "nodekey":NodeKey, "OID":6,'walletID':session['walletID'],'lastTransID':lastTransID}
	rows[2] =paxos(2,dict_data)
	
	if rows[2] != 'Fail' and json.loads(rows[2]) != []:
		rows[3] = rows[3]+tuple(json.loads(rows[2]))
		dict_data['lastTransID'] = int(json.loads(rows[2])[0][0])
		
	rows[1] =paxos(1,dict_data)
	if rows[1] != 'Fail' and json.loads(rows[1]) != []:
		rows[3] = rows[3] +tuple(json.loads(rows[1]))
#%%%   PAXOS HERE   %%%
...
#%%%  PAXOS HERE  %%%
			...
			conn.commit()
			dict_data = {'NID':3, "nodekey":NodeKey, "OID":7 ,'walletID':session['walletID']}
			paxos(2,dict_data)
			paxos(1,dict_data)
#%%%  PAXOS HERE  %%%
...
#%%%   PAXOS HERE   %%%
			...
			return render_template('walletHome.html', message='Status Refreshed !!', session=session, rows =rows[3], form=form, form2=form2, form3=form3)
...
#%%%  PAXOS HERE  %%%
			...
			if request.form["password"] != tmpPass:
				return render_template('walletHome.html', message = "Your Password is Incorrect. Please Try Again !!", 
															           session=session, rows =rows[3], form = form,form2= form2, form3=form3)
...
#%%%  PAXOS HERE  %%%
				...
				if float(request.form['amount'])>float(tmpAmount):
...
#%%%  PAXOS HERE  %%%
					...
					dict_data = {'NID':3,'nodekey':NodeKey,'OID':10}
					update = paxos(2,dict_data)
...
#%%%  PAXOS HERE  %%%
					...
					dict_data ={'NID':3,'nodekey':NodeKey,'OID':11,'transactionID':tmpID,'walletID':session['walletID'],
																	'receiverID':request.form['receiverID'],'amount':request.form['amount']}

					tmpconsent = [paxos(2,dict_data),paxos(1,dict_data)]												
					if 'OK' in tmpconsent:
							conn.commit()
					else:
						...
						dict_data={'NID':3,'nodekey':NodeKey,'OID':12,'op1':op1,'op2':op2,'op3':op3}
						paxos(2,dict_data)
						paxos(1,dict_data)
						session.clear()
						return render_template('invalid.html')
...
#%%%   PAXOS HERE   %%%
					...
					return render_template('walletHome.html', message=tmpMessage, session=session, rows =rows[3], form=form, form2=form2, form3=form3)

	return render_template('walletHome.html', message = "", session=session, rows=rows[3], form = form,form2= form2, form3=form3)
...
#%%%   PAXOS HERE   %%%
	...
	dict_data = {'NID':3, "nodekey":NodeKey, "OID":9,'lastTransID':lastTransID}
	rows[2] =paxos(2,dict_data)
	
	if rows[2] not in [None,'Fail']:
		rows[3] = rows[3]+tuple(json.loads(rows[2]))
		dict_data['lastTransID'] = int(json.loads(rows[2])[0][0])
		rows[1] =paxos(1,dict_data)

	if rows[1]  not in [None,'Fail']:
		rows[3] = rows[3] +tuple(json.loads(rows[1]))
#%%%   PAXOS HERE   %%%
	...
	return render_template('access.html', rows = rows[3])
