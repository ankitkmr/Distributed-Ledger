...
#%%%  PAXOS HERE  %%%
		tmpData =[]
		dict_data = {'NID':2,'nodekey':NodeKey,'OID':2,"walletID":session['walletID']}
...
#%%%  PAXOS HERE  %%%
			dict_data = {'NID':2,'nodekey':NodeKey,'OID':1}
...
# %%%   PAXOS HERE  %%%
			...
			dict_data={'NID':2,'nodekey':NodeKey,'OID':3, 'walletID':session['walletID'],'name':session['name'],
			...
			if tmpData[1] == 'OK' or tmpData[3] == 'OK':
				#consensus
				...
				dict_data = {'NID':2,'nodekey':NodeKey,'OID':12,'rollback':tmpCommand}
				paxos(1,dict_data)
				paxos(3,dict_data)
				session.clear()
...
#%%%  PAXOS HERE  %%%
			...
			if data in [None,'Fail']:
				dict_data = {'NID':2, "nodekey":NodeKey, "OID":4,'walletID':request.form['walletID'],'password':request.form['password']}
...
#%%%  PAXOS HERE  %%%
				dict_data = {'NID':2, "nodekey":NodeKey, "OID":5,'walletID':session['walletID']}
				tmpData = {}
				tmpData[1]= paxos(1,dict_data)
				tmpData[3]= paxos(3,dict_data)
...
#%%%   PAXOS HERE   %%%
	...
	rows = {}
	rows[2] = cursor.fetchall()
	if rows[2] is ():
		lastTransID=0
	else:
		lastTransID=int(rows[2][0][0])

	dict_data = {'NID':2, "nodekey":NodeKey, "OID":6,'walletID':session['walletID'],'lastTransID':lastTransID}
	rows[1] =paxos(1,dict_data)
	
	if rows[1] != 'Fail' and json.loads(rows[1])!=[]:
		rows[2] = rows[2]+tuple(json.loads(rows[1]))
		dict_data['lastTransID'] = int(json.loads(rows[1])[0][0])
		
	rows[3] =paxos(3,dict_data)
	if rows[3] != 'Fail' and json.loads(rows[3])!=[]:
		rows[2] = rows[2] +tuple(json.loads(rows[3]))
#%%%   PAXOS HERE   %%%
...
#%%%  PAXOS HERE  %%%
			...
			conn.commit()
			dict_data = {'NID':2, "nodekey":NodeKey, "OID":7 ,'walletID':session['walletID']}
			paxos(1,dict_data)
			paxos(3,dict_data)
#%%%  PAXOS HERE  %%%
...
#%%%   PAXOS HERE   %%%
			...
			return render_template('walletHome.html', message='Status Refreshed !!', session=session, rows =rows[2], form=form, form2=form2, form3=form3)
...
#%%%  PAXOS HERE  %%%
					...
					dict_data = {'NID':2,'nodekey':NodeKey,'OID':10}
					update = paxos(1,dict_data)
...
#%%%  PAXOS HERE  %%%
					...
					dict_data ={'NID':2,'nodekey':NodeKey,'OID':11,'transactionID':tmpID,'walletID':session['walletID'],
																	'receiverID':request.form['receiverID'],'amount':request.form['amount']}

					tmpconsent = [paxos(1,dict_data),paxos(3,dict_data)]												
					if 'OK' in tmpconsent:
							conn.commit()
					else:
						...
						dict_data={'NID':2,'nodekey':NodeKey,'OID':12,'op1':op1,'op2':op2,'op3':op3}
						paxos(1,dict_data)
						paxos(3,dict_data)
						session.clear()
						return render_template('invalid.html')
...
#%%%   PAXOS HERE   %%%
					...
					return render_template('walletHome.html', message=tmpMessage, session=session, rows =rows[2], form=form, form2=form2, form3=form3)

	return render_template('walletHome.html', message = "", session=session, rows=rows[2], form = form,form2= form2, form3=form3)
...
#%%%   PAXOS HERE   %%%
	...
	dict_data = {'NID':2, "nodekey":NodeKey, "OID":9,'lastTransID':lastTransID}
	rows[1] =paxos(1,dict_data)
...
#%%%   PAXOS HERE   %%%
	...
	return render_template('access.html', rows = rows[2])
